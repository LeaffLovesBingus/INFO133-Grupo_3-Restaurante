...
def generar_ventas(cur, fake, fecha_inicio, fecha_fin, cantidad):
...
def generar_reservas(cur, fake, fecha_inicio, fecha_fin, cantidad):
    mesas = obtener_ids(cur, "Mesas", "Id_Mesa")
...
def generar_datos_masivos():
    try:
        ...
        rangos = [
            (hoy - relativedelta(months=i), hoy - relativedelta(months=i) + relativedelta(months=1) - timedelta(seconds=1))
            for i in range(36)
        ]

        for inicio, fin in tqdm(rangos[::-1], desc="Cargando datos por mes"):
            generar_ventas(cur, fake, inicio, fin, 500)
            generar_reservas(cur, fake, inicio, fin, 500)
            conn.commit()
